...
function backwardsPrime(start, stop){
    let backwards_primes = [];
    for (let x = start; x <= stop; x++) {
        if (x <= 1) continue;
        if (isPrime(x)) {
            let y = parseInt(x.toString().split('').reverse().join(''));
            if (isPrime(y) && (x != y)) backwards_primes.push(x);
        }
    } 
    return backwards_primes.sort(function(a, b){return a-b})
}
...
console.log(backwardsPrime(98863,100136))
