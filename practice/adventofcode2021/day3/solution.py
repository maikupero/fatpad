# Static variables for handling input
columns = 12
rows = 1000
report = []

# Fill our list with the input data - array of array of binary numbers.
...
def part1(report):
	...
	for x in range(columns):
		binary = 2**((columns - 1) - x)
		gamsum += gamma[x] * binary
		epsum += epsilon[x] * binary
...
def part2(report):
	...
	while len(oxygen) > 1:
		for x in range(columns):
			# Have to go through the column once to determine numerousbit
			ones = 0
			...
			if ones >= (len(oxygen) / 2):
				...
				numerousbit = 0
			# Go through again logging the indices of the rows we'll have to remove based on numerousbit.
			toremove = []
			for y in range(len(oxygen)):
				if oxygen[y][x] != numerousbit:
					toremove.append(y)
			# Go from the end of the list so as not to screw with out of range indices.
			for index in range(len(toremove)-1, -1, -1):
				oxygen.pop(toremove[index])
			if len(oxygen) == 1:
				break
					
	while len(co2) > 1:
		for x in range(columns):
			# Have to go through the column once to determine rarebit
			ones = 0
			...
			if ones < (len(co2) / 2):
				...
				rarebit = 0
			# Go through again logging the indices of the rows we'll have to remove based on rarebit.
			toremove = []
			for y in range(len(co2)):
				if co2[y][x] != rarebit:
					toremove.append(y)
	...
	for x in range(columns):
		binary = 2**((columns - 1) - x)
		oxygenrating += oxygen[0][x] * binary
		co2rating += co2[0][x] * binary
...
# 			for y in range(len(oxygen)):
# 				ones += oxygen[y][x]
# 				print(f'x,y,val,ones: {x},{y},{oxygen[y][x]},{ones}')
# 			if ones >= (len(oxygen) / 2):
# 				numerousbit = 1
# 			else:
# 				numerousbit = 0
# 			print(f'numerous bit: {numerousbit}')
# 			# Go through again logging the indices of the rows we'll have to remove based on numerousbit.
# 			toremove = []
# 			for y in range(len(oxygen)):
# 				print(f'x,y,val,ones: {x},{y},{oxygen[y][x]},{ones}')
# 				if oxygen[y][x] != numerousbit:
# 					toremove.append(y)
# 			print(f'Uncut oxygen: {oxygen}')
# 			print(f'Indices to cut: {toremove}')
# 			# Go from the end of the list so as not to screw with out of range indices.
# 			for index in range(len(toremove)-1, -1, -1):
# 				oxygen.pop(toremove[index])
# 			print(f'Final oxygen: {oxygen}')
# 			if len(oxygen) == 1:
# 				break
...
# 			for y in range(len(co2)):
# 				ones += co2[y][x]
# 				print(f'x,y,val,ones: {x},{y},{co2[y][x]},{ones}')
# 			if ones < (len(co2) / 2):
# 				rarebit = 1
# 			else:
# 				rarebit = 0
# 			print(f'rarebit: {rarebit}')
# 			# Go through again logging the indices of the rows we'll have to remove based on rarebit.
# 			toremove = []
# 			for y in range(len(co2)):
# 				print(f'x,y,val,ones: {x},{y},{co2[y][x]},{ones}')
# 				if co2[y][x] != rarebit:
# 					toremove.append(y)
# 			print(f'Uncut co2: {co2}')
# 			print(f'Indices to cut: {toremove}')
# 			# Go from the end of the list so as not to screw with out of range indices.
# 			for index in range(len(toremove)-1, -1, -1):
# 				co2.pop(toremove[index])
# 			print(f'Final co2: {co2}')
# 			if len(co2) == 1:
# 				break
